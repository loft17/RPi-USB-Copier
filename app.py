...
class DatePickerDialog(CustomDialog):
    def __init__(self, parent, title="Seleccionar Fecha", initial_date=None):
        ...
        # MODIFICADO: Ajustar el ratio si es necesario, aunque 0.75 debería ser suficiente
        # para acomodar los elementos lado a lado, dado que el ancho es generoso.
        super().__init__(parent, title, fixed_height_ratio=0.65) # Un poco menos alto para acomodar botones al lado
        self._create_body()
        # MODIFICADO: La acción de cerrar la ventana (botón 'X') ahora actúa como "Cancelar"
    ...
    def _create_custom_spinbox(self, parent_frame, label_text, value_type):
        ...
        # MODIFICADO: Ajustar width del label para dar más espacio
        ...
        value_label = ttk.Label(frame, text="00", style="DateValue.Dialog.TLabel", width=4, anchor="center"); value_label.pack(side=tk.LEFT, padx=2) # Menos padx horizontal
    ...
    def _create_body(self):
        # MODIFICADO: Reestructurar para tener selectores a la izquierda y botones a la derecha
        body_container = ttk.Frame(self.main_frame, style="CustomDialog.TFrame")
        body_container.pack(expand=True, fill=tk.BOTH, pady=5, padx=5) # Menos pady vertical

        body_container.columnconfigure(0, weight=2) # Columna para selectores de fecha (más ancha)
        body_container.columnconfigure(1, weight=1) # Columna para botones (más estrecha)
        body_container.rowconfigure(0, weight=1)    # Fila única que se expande verticalmente

        # Frame para los selectores de fecha (izquierda)
        ...
        # Centrar verticalmente los selectores en su frame (opcional, o dejar que se apilen desde arriba)
        date_selectors_frame.rowconfigure(0, weight=1) # Espacio arriba
        date_selectors_frame.rowconfigure(1, weight=0) # Año
        date_selectors_frame.rowconfigure(2, weight=0) # Mes
        date_selectors_frame.rowconfigure(3, weight=0) # Día
        date_selectors_frame.rowconfigure(4, weight=1) # Espacio abajo
        ...
        year_frame.grid(row=1, column=0, sticky="ew", pady=2) # Menos pady
        ...
        buttons_frame.grid(row=0, column=1, sticky="ns", padx=(5, 0)) # sticky ns para centrar verticalmente los botones

        # Configurar filas para centrar los botones verticalmente dentro de su frame
        buttons_frame.rowconfigure(0, weight=1)  # Espacio flexible arriba
        buttons_frame.rowconfigure(1, weight=0)  # Botón Aceptar
        buttons_frame.rowconfigure(2, weight=0)  # Botón Cancelar
        buttons_frame.rowconfigure(3, weight=1)  # Espacio flexible abajo
        buttons_frame.columnconfigure(0, weight=1) # Para que los botones puedan expandirse (ew)

        accept_button = ttk.Button(buttons_frame, text="Aceptar", command=self.apply, style="Dialog.TButton", width=8) # width más pequeño
        accept_button.grid(row=1, column=0, pady=(0, 4), sticky="ew") # Menos pady, sticky ew
    ...
    def cancel(self): self.result = None; self.destroy() # Restaurado cancel
...
class DeviceSelectionDialog(CustomDialog):
    ...
    def _create_body(self):
        ...
        if self.devices and len(self.devices) <= 2:
            for device in self.devices:
                ...
                btn = ttk.Button(body_content_frame, text=btn_text,
                                 style="LargeDevice.Dialog.TButton",
                                 command=lambda d=device: self.on_device_button_select(d))
                btn.pack(fill=tk.X, pady=5, ipady=8)
            self._create_button_bar([("Cancelar", self.on_cancel, "Dialog.TButton")])
        elif self.devices:
            self.listbox_frame = ttk.Frame(body_content_frame, style="CustomDialog.TFrame")
            self.listbox_frame.pack(pady=5, padx=5, fill=tk.BOTH, expand=True)

            self.listbox = tk.Listbox(self.listbox_frame, width=35, height=4,
                                      bg=BORDER_COLOR, fg=FG_COLOR,
                                      selectbackground=ACCENT_COLOR, selectforeground=FG_COLOR,
                                      font=(FONT_FAMILY, FONT_SIZE_NORMAL),
                                      borderwidth=0, highlightthickness=0, relief=tk.FLAT, activestyle="none")
            for dev in self.devices:
                self.listbox.insert(tk.END, f" {dev['label']} ({dev['free_space_gb']:.1f}G/{dev['total_space_gb']:.1f}G)")
    ...
    def on_device_button_select(self, device):
        self.result = device
        self.destroy()
...
class SystemMenuDialog(CustomDialog):
    ...
    def _create_menu_buttons(self):
        ...
        menu_frame.pack(expand=True, fill=tk.BOTH, pady=10, padx=10)

        btn_close_app = ttk.Button(menu_frame, text="Cerrar Aplicación", command=self.close_application, style="Large.TButton")
        btn_close_app.pack(fill=tk.X, pady=5, ipady=5)

        btn_reboot = ttk.Button(menu_frame, text="Reiniciar Pi", command=self.reboot_system, style="Danger.Large.TButton")
        btn_reboot.pack(fill=tk.X, pady=5, ipady=5)

        btn_shutdown = ttk.Button(menu_frame, text="Apagar Pi", command=self.shutdown_system, style="Danger.Large.TButton")
        btn_shutdown.pack(fill=tk.X, pady=5, ipady=5)
...
class ModernCopierApp:
    ...
    def setup_styles(self):
        ...
        self.style.configure("LargeDevice.Dialog.TButton", font=(FONT_FAMILY, FONT_SIZE_NORMAL), padding=(8, 8),
                             background=ACCENT_COLOR, foreground=FG_COLOR, justify=tk.CENTER, anchor=tk.CENTER)
    ...
    def handle_copy_cancelled(self):
        ...
        if hasattr(self, 'target_copy_folder_full_path') and self.target_copy_folder_full_path and os.path.exists(self.target_copy_folder_full_path):
            try:
                is_empty = True
                with os.scandir(self.target_copy_folder_full_path) as it:
    ...
    def finalize_copy_ui(self, success, files_copied=0, total_files_attempted=0, message_override=None):
        ...
        if self.source_device_info and os.path.ismount(self.source_device_info['path']):
            try:
                usage = psutil.disk_usage(self.source_device_info['path'])
                self.source_device_info['free_space_gb'] = usage.free / (1024**3)
                self._display_device_info(self.source_info_label, self.source_device_info, "Origen")
            except: pass
        if self.destination_device_info and os.path.ismount(self.destination_device_info['path']):
            try:
                usage = psutil.disk_usage(self.destination_device_info['path'])
                self.destination_device_info['free_space_gb'] = usage.free / (1024**3)
                self._display_device_info(self.dest_info_label, self.destination_device_info, "Destino")
            except: pass
